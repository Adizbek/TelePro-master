...
public class MessagesStorage {
                        ...
                        if (load_type == -2) {
                            ...
                            if (exist = cursor.next()) {
                                ...
                                if (data != null) {
                                    ...
                                    data.reuse();
                                    if (oldMessage != null) {
                                        updateEditedMessage(oldMessage);
    ...
    // sadegh // edits
    private void updateEditedMessage(TLRPC.Message oldMessage) {
        if (ApplicationLoader.applicationContext == null)
            return;

        int id = oldMessage.to_id.channel_id+ oldMessage.to_id.chat_id + oldMessage.to_id.user_id;
        SharedPreferences sharedPreferences = ApplicationLoader.applicationContext.getSharedPreferences("edits", Context.MODE_PRIVATE);
        String editedMessage = sharedPreferences.getString("edit_d" +id+ "_m" + oldMessage.id, " ");
        int time = sharedPreferences.getInt("time_d" + id + "_m" + oldMessage.id, 1);


        String str = oldMessage.message;


        editedMessage = editedMessage + "\n\n Edit " + time + " :\n\n" + str;
        sharedPreferences.edit().putString("edit_d" + id + "_m" + oldMessage.id, editedMessage).commit();
        sharedPreferences.edit().putInt("time_d" +id + "_m" + oldMessage.id, time + 1).commit();

    }


    // sadegh // deleted
    private void updateDeletedMessage(int channelID , String mids) {
        if (ApplicationLoader.applicationContext == null)
            return;
        ...
        for (int i = 0; i < ids.length ; i++) {
            ...
            SharedPreferences sharedPreferences = ApplicationLoader.applicationContext.getSharedPreferences("edits", Context.MODE_PRIVATE);
            sharedPreferences.edit().putBoolean("deleted_"+channelID +"_"+id ,true ).commit();
        }

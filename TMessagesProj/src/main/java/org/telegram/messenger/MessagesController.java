...
import sections.oneSignalclasses.FcmService.Setting.hideChannelController;
...
//            if (LastInListController.is(dialog2)) return -1;
    ...
    public MessagesController() {
        ...
        //Sadegh hideLeftGroup
        SharedPreferences Prefs = ApplicationLoader.applicationContext.getSharedPreferences("BaseConfig", Activity.MODE_PRIVATE);
        ...
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            if (hide != -1) {
                if (hide == 0123) {
                    ...
                    if (!hid.contains(d.id)) {
                        hid.add(d.id);
                        dialogsSpecialHides.add(d);
                    }
                } else if (hide == 0124) {

                    if (!hid.contains(d.id)) {
                        hid.add(d.id);
                        dialogsSpecialBots.add(d);
                    }
                }
                dialogs.remove(d);
            } else {
                ...
                if (Favourite.getInstance().isFavorite(d.id)) {
                    dialogsFavs.add(d);
                }
                if (d.unread_count > 0) {
                    dialogsUnread.add(d);
...
//        }
    ...
    public TLRPC.User getUser(String username) {
        ...
        return (TLRPC.User) objectsByUsernames.get(username.toLowerCase());
    }
